...
public class UnityCommunicatorClient
{
    ...
    //Returns true, when every GameObject was set to the received parameters, else false
    {
        ...
        if (sceneShotProcessed == false && readyToCapture)
        {
            return true;
        }
        else
    ...
    {
        if (this.sceneShotProcessed == false && this.objectParametersSet[objectID] == false)
        {
            if (objectID == -1)
            {
                Debug.Log("ObjectID not initialized. Please assign objectID in inspector.");
            }
